...
*********************************************/
	buffer += "<input type='hidden' name='client_id' id='client_id' value='"+depth0.clientId+"' />";
	buffer += "<input type='hidden' name='baseUrl' id='baseUrl' value='"+depth0.baseUrl+"' />";
/*********************************************
...
**	Lees Edit to swagger
*********************************************/
var len = $("#resources>li").length;
	foundHelper = helpers.globals;
	stack1 = foundHelper || depth0.globals;
	if(typeof stack1 != "undefined"){
		buffer += "<li class='global'>";
buffer+= "<div class='header' style='height:27px;display:block;background:#f7edf6;border:1px solid #e8c6e3;'><span style='background:#a41e90;border-radius:2px;padding: 10px 0 7px 0;float:left;text-align: center;font-size: 0.7em;color: white;text-transform: uppercase;width:50px;'>Globals<span></div>";
buffer+= "<div class='content' style='background:#f7edf6;border:1px solid #e8c6e3;margin-bottom:10px;border-top:none;padding: 10px;'>";


buffer +='<table class="fullwidth"><thead><tr><th>Parameter</th><th>Value</th><th>Description</th></tr></thead><tbody class="operation-params">';

		for(x in stack1){
			log(stack1[x].name+" = "+stack1[x].type+" -> "+len);
buffer += "<tr><td class='code required'>"+stack1[x].name+"</td><td><input id='"+stack1[x].unique_name+"' type='"+stack1[x].type+"' /></td><td width='500'><strong>Global variable.</strong></td></tr>";
		}
		buffer +='</tbody></table>'
		buffer += "</div>"
		buffer += "</li>"
...
*********************************************/
    ...
    HeaderView.prototype.showCustom = function(e) {
      ...
      return this.trigger('update-swagger-ui', {
        discoveryUrl:'http://127.0.0.1/generated_cms/resources.json',
  ...
  ResourceView = (function(_super) {
	...
	//	log("this.model = "+Handlebars.templates.resource(this.model));
...
if(typeof this.model.globals != "undefined"){
	log("globals = "+this.model.globals);
	for(variable in this.model.globals){
		log("global variable = "+this.model.globals[variable].name);
		invocationUrl=invocationUrl.replace("{"+this.model.globals[variable].name+"}",$("#"+this.model.globals[variable].unique_name).val());
